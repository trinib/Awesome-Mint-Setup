# WSL VHDX Complete Backup & Restore Guides
...
Run all commands from a Windows **PowerShell** window opened as an Administrator (not inside the Linux terminal).

---

## Method 1: The Raw File Copy (`ext4.vhdx`)

Use this method to manually copy the raw virtual disk file. It is the fastest way to backup and allows for an instant restore.

### Phase 1: The Backup
1. **Shut down WSL** completely to prevent data corruption:
   ```powershell
   wsl --shutdown
   ```
2. **Locate the source file** by navigating to this hidden folder path:
   ```text
   C:\Users\<Your_Windows_Username>\AppData\Local\Packages\
   ```
   Look for your Linux distribution folder (e.g., `CanonicalGroupLimited.Ubuntu...` or `TheDebianProject...`) and open the **`LocalState`** directory inside it.
3. **Copy the file** named `ext4.vhdx` and paste it directly into your safe backup directory:
   ```text
   Destination Example: D:\Backups\WSL\ext4.vhdx
   ```
...
1. **Initialize the WSL platform** on the system:
   ...
   ```
2. **Import the file in-place** to instantly register the drive exactly where it sits on your backup storage:
   ```powershell
